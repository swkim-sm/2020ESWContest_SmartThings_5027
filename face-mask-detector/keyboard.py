...
def check_ROI(point, btn_roi):
    ...
    if point[0]+500 >= btn_roi[0] and point[0]+500 <= btn_roi[2]: X_CORRECT = True
    else: X_CORRECT = False

    if point[1] >= btn_roi[1] and point[1] <= btn_roi[3]: Y_CORRECT = True
...
while True:
    ...
    # mask 착용 확인 후 virtual keyboard 조작 단계
        ...
        keyboard_x = 600
        keyboard_y = 200
        keyboard_roi = frame[0:1100, 500:1100]
        ...
        if click_flag: #클릭 발생 시
            for btn in range(0, len(btn_list)):
                if check_ROI(center, btn_list[btn]):
                    make_button(btn_list[btn][0], btn_list[btn][1], btn_list[btn][2]-btn_list[btn][0], btn_list[btn][3]-btn_list[btn][1], btn_list[btn][4], True)
                    print(btn)
